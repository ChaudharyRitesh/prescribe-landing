...
export async function getAboutSection(): Promise<AboutSection | null> {
  try {
    const data = await client.fetch(
      `*[_type == "aboutSection" && isActive == true][0]`,
      {},
      { next: { revalidate: 0 } }
    );
...
export async function getModules(): Promise<any[]> {
  try {
    const data = await client.fetch(
      `*[_type == "moduleSection" && isActive == true] | order(order asc)`,
      {},
      { next: { revalidate: 0 } }
    );
    return data;
...
export async function getComparisons(): Promise<any[]> {
  try {
    const data = await client.fetch(
      `*[_type == "comparisonSection" && isActive == true] | order(order asc)`,
      {},
      { next: { revalidate: 0 } }
    );
    return data;
...
export async function getTrustItems(): Promise<any[]> {
  try {
    const data = await client.fetch(
      `*[_type == "trustSection" && isActive == true] | order(order asc)`,
      {},
      { next: { revalidate: 0 } }
    );
    return data;
